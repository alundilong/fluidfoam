...
rhof_phi = rhof*phi*0
rho_nu = rho1*nu1
rhof_nuf = np.zeros(total_surface_array_size)
rhof_nuf[n_internal_faces:n_internal_faces+total_boundary_face_size] = rho_nu[n_cells:n_cells+total_boundary_face_size]
for facei in range(len(neighfile.values)):
    celli_o = ownerfile.values[facei]
    celli_n = neighfile.values[facei]
    wn = weights[facei]
    wo = 1.0 - wn
    rhof_nuf[facei] = wo*rho_nu[celli_o] + wn*rho_nu[celli_n]
...
snGradU_x = np.zeros(total_surface_array_size)
snGradU_y = np.zeros(total_surface_array_size)
snGradU_z = np.zeros(total_surface_array_size)
...
for facei in range(len(ownerfile.values)):
    ...
    if facei < n_internal_faces:
        ...
        snGradU_x[facei] = delta*(Ux1[celli_n]-Ux1[celli_o])
        snGradU_y[facei] = delta*(Uy1[celli_n]-Uy1[celli_o])
        snGradU_z[facei] = delta*(Uz1[celli_n]-Uz1[celli_o])
    else:
        ...
        snGradU_x[facei] = delta*(Ufx[facei]-Ux1[celli_o])
        snGradU_y[facei] = delta*(Ufy[facei]-Uy1[celli_o])
        snGradU_z[facei] = delta*(Ufz[facei]-Uz1[celli_o])
...
deltaN = 1.0e-8/np.power(np.average(vols),1.0/3.0)
print(f">> {deltaN}")
n_hatf_x = gradAlphaxf/(magGradAlphaf+deltaN)
n_hatf_y = gradAlphayf/(magGradAlphaf+deltaN)
n_hatf_z = gradAlphazf/(magGradAlphaf+deltaN)

kappa = np.zeros(total_vol_array_size)
n_hatf = n_hatf_x*Sfx + n_hatf_y*Sfy + n_hatf_z*Sfz

#kappa = -div(n_hatf)
for facei in range(len(ownerfile.values)):
    celli_o = ownerfile.values[facei]
    kappa[celli_o] += -(n_hatf_x[facei]*Sfx[facei] + n_hatf_y[facei]*Sfy[facei] + n_hatf_z[facei]*Sfz[facei])
    #kappa[celli_o] += -n_hatf[facei]

    if facei < n_internal_faces:
        celli_n = neighfile.values[facei]
        kappa[celli_n] += (n_hatf_x[facei]*Sfx[facei] + n_hatf_y[facei]*Sfy[facei] + n_hatf_z[facei]*Sfz[facei])
        #kappa[celli_n] += n_hatf[facei]
kappa[:n_cells] = kappa[:n_cells]/vols

for facei in range(len(neighfile.values),len(ownerfile.values),1):
    celli_o = ownerfile.values[facei]
    vol_facei = facei - n_internal_faces + n_cells
    kappa[vol_facei] = kappa[celli_o]
...
for facei in range(len(ownerfile.values)):
    ...
    if facei < n_internal_faces:
        ...
        Hz[celli_o] += rhof_nuf[facei]*delta*magSf[facei]*Uz1[celli_n]
        # dev2(owner)
        Hx[celli_o] += rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_xx + Sfy[facei]*dev2_gradU_t_xy + Sfz[facei]*dev2_gradU_t_xz)
        Hy[celli_o] += rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_yx + Sfy[facei]*dev2_gradU_t_yy + Sfz[facei]*dev2_gradU_t_yz)
        Hz[celli_o] += rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_zx + Sfy[facei]*dev2_gradU_t_zy + Sfz[facei]*dev2_gradU_t_zz)

        celli_n = neighfile.values[facei]
        # convection (neigh)
        Hx[celli_n] += rhof_phi[facei]*wo*Ux1[celli_o]
        Hy[celli_n] += rhof_phi[facei]*wo*Uy1[celli_o]
        Hz[celli_n] += rhof_phi[facei]*wo*Uz1[celli_o]
        # conduction (neigh)
        Hx[celli_n] += rhof_nuf[facei]*delta*magSf[facei]*Ux1[celli_o]
        Hy[celli_n] += rhof_nuf[facei]*delta*magSf[facei]*Uy1[celli_o]
        Hz[celli_n] += rhof_nuf[facei]*delta*magSf[facei]*Uz1[celli_o]
        # dev2(neigh)
        Hx[celli_n] += -rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_xx + Sfy[facei]*dev2_gradU_t_xy + Sfz[facei]*dev2_gradU_t_xz)
        Hy[celli_n] += -rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_yx + Sfy[facei]*dev2_gradU_t_yy + Sfz[facei]*dev2_gradU_t_yz)
        Hz[celli_n] += -rhof_nuf[facei]*(Sfx[facei]*dev2_gradU_t_zx + Sfy[facei]*dev2_gradU_t_zy + Sfz[facei]*dev2_gradU_t_zz)
    # (boundary contribution)
    else:
        vol_facei = facei-n_internal_faces+n_cells
        U_ref_x = Ufx[facei]
        ...
        #g_ref_x = gradUxxf[facei]*nx[facei] + gradUxyf[facei]*nx[facei] + gradUxzf[facei]*ny[facei]
        #g_ref_y = gradUyxf[facei]*nx[facei] + gradUyyf[facei]*nx[facei] + gradUyzf[facei]*ny[facei]
        #g_ref_z = gradUzxf[facei]*nx[facei] + gradUzyf[facei]*nx[facei] + gradUzzf[facei]*ny[facei]
...
rhoUByDeltaT_x_oldTime = rho0*Ux0/dt
rhoUByDeltaT_y_oldTime = rho0*Uy0/dt
rhoUByDeltaT_z_oldTime = rho0*Uz0/dt
...
for facei in range(len(neighfile.values)):
    ...
    HByA_f_x[facei] = wn*HByA_x[celli_n] + wo*HByA_x[celli_o]
    HByA_f_y[facei] = wn*HByA_y[celli_n] + wo*HByA_y[celli_o]
    HByA_f_z[facei] = wn*HByA_z[celli_n] + wo*HByA_z[celli_o]
...
phiPrgh_no_rAf = snGradPrgh*magSf
phiHByA = Sfx*HByA_f_x + Sfy*HByA_f_y + Sfz*HByA_f_z
phig = -(gx*Cfx + gy*Cfy + gz*Cfz)*snGradRho*magSf*rAf
phig_no_rAf = -(gx*Cfx + gy*Cfy + gz*Cfz)*snGradRho*magSf
phiSurfaceTension = kappaf*snGradAlpha*sigma*rAf*magSf
phiSurfaceTension_no_rAf = kappaf*snGradAlpha*sigma*magSf
sigmaK = sigma*kappa
sigmaKf = sigma*kappaf
...
writeScalarVolType(kappa,n_cells,bounfile,time,sol,"K")
writeScalarVolType(sigmaK,n_cells,bounfile,time,sol,"sigmaK")
...
writeScalarSurfaceType(nuf,n_internal_faces,bounfile,time,sol,"nuf")
writeScalarSurfaceType(rhof_phi,n_internal_faces,bounfile,time,sol,"rhoPhi")
writeScalarSurfaceType(rhof_nuf,n_internal_faces,bounfile,time,sol,"rhoNuf")
...
writeScalarSurfaceType(phig_no_rAf,n_internal_faces,bounfile,time,sol,"phig_no_rAf")
writeScalarSurfaceType(phiSurfaceTension,n_internal_faces,bounfile,time,sol,"phiSurfaceTension")
writeScalarSurfaceType(phiSurfaceTension_no_rAf,n_internal_faces,bounfile,time,sol,"phiSurfaceTension_no_rAf")
...
writeScalarSurfaceType(phiPrgh_no_rAf,n_internal_faces,bounfile,time,sol,"phiPrgh_no_rAf")
...
writeScalarSurfaceType(magGradAlphaf,n_internal_faces,bounfile,time,sol,"magGradAlphaf")
